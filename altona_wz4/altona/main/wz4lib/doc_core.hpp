...
// The GUI-free half of doc.hpp: the document, page, operator, class and type
// model, the command list and the executive. Everything here compiles with
// no window system, no gui library and no shader library.
//
// The viewport painting context (wPaintInfo), the parameter panel builder
// (wGridFrameHelper) and the custom editor base class (wCustomEditor) live in
// doc_gui.hpp. wType and wClass still refer to them, but only through
// references in virtual signatures and through function pointers, so a
// forward declaration is enough and nothing here ever dereferences them.
//
// doc.hpp includes both halves, so existing consumers are unaffected.

#ifndef FILE_WERKKZEUG4_DOC_CORE_HPP
#define FILE_WERKKZEUG4_DOC_CORE_HPP
...
#include "base/graphics.hpp"      // sViewport, sTargetSpec
#include "gui/treeinfo.hpp"       // sListWindowTreeInfo — pure data, no gui
#include "gui/theme.hpp"          // sGuiTheme — pure data, no gui
