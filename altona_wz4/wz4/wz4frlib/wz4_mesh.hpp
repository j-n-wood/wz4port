...
// wz4port: was wz4frlib/wz4_mtrl2.hpp. This header only needs `Wz4Mtrl *Mtrl` —
// a pointer, in Wz4MeshCluster — so a forward declaration is enough, and the
// full header cannot be included headlessly anyway: it reaches
// wz4_mtrl2_shader.hpp, which is generated by the asc shader compiler this port
// does not build. See wz4port/patches/10-mesh-headless.md.
#if WZ4PORT_HEADLESS_MTRL
class Wz4Mtrl;
// Displace() takes an sImageI16 *, which this header was getting transitively
// through wz4_mtrl2.hpp. Declaring what it uses is the fix either way — relying
// on a transitive include is how a header comes to depend on the whole world.
